...
public class EnemyMago : MonoBehaviour
{
    ...
    public float distanciaOptima = 6f;

    [Header("Disparo")]
    public float cooldownDisparo = 2f;
    public float danioProyectil = 10f;
    ...
    void Update()
    {
        ...
        if (!enRetroceso && Time.time >= tiempoUltimoDisparo + cooldownDisparo)
            Disparar();
    }
    ...
    private void CalcularRuta()
    {
        ...
        if (distancia < distanciaHuida)
        {
            ...
            destino = rb.position + direccionHuida * distanciaOptima;
        }
    ...
    private void Disparar()
    {
        ...
        if (p != null)
            p.Inicializar(direccion, danioProyectil);
    }
}